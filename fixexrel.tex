...
\begin{abstract}
We give an explicit description of all the exchange relations for finite type cluster algebras with acyclic initial seed and principal coefficients.
\end{abstract}
...
\section{Introduction}
  ...
  \saySS{Change this if Nathan decides not to participate.}
  ...
  \end{theorem}
  \saySS{I am not sure it is worth to state this as a theorem; as is it makes much easier to restate the result later but it is extremely imprecise to call it a theorem.}
  \say[NR]{My vote (and it's going to end up that I don't have a vote) is not to state it as a theorem.  This is a variation that I haven't seen yet, but it reminds me of something that I've seen in a few papers that drives me crazy:  "Theorem: there exists an explicit bijection between BLAH1 and BLAH2."  Usually that happens when we already knew the counts were equal, and I think that statement is essentially meaningless.  The right thing is to define a map and say "Theorem:  My map is a bijection."  In this case, I think the right thing to do is \cref{thm:main} (possibly modulo some suggestions below.)}
  
  
  The claim in the previous statement lies \say[NR]{Changed "lays" to "lies".  This is confusing even for native English speakers, but "lies" is correct here.}
  ...
  Let $Q$ be the \emph{root lattice} of $A$ with simple roots $\alpha_i$; as for $P$, we will routinely think of elements in $Q$ as integer vectors using the basis of simple roots.
  \begin{definition}
  ...
  \begin{proposition}\say[NR]{This theorem should be cited to a specific result (if possible---this kind of statement has been really hard for me to track down in the past!}
...
\say[NR]{Some thoughts on how to sell this:  
\cref{thm:main} sounds less cool than it is because you already stated \cref{prop:coeff-free,cor:A0}.
How about this:  State the coolest possible theorem all at once, and then mention that parts of it already appeared and say what is the new piece.
So, that would mean:
First state the piece of \cref{prop:coeff-free} that defines $\uplus$.
Then state a theorem that says given exchangeable weights, the exchange relation is BLAH where $\alpha$ is BLAH.
THEN, point out that your previous awesome result showed that the coefficient-free exchange relations were of a certain form.
Depending on how much you want to sound like you're giving a history, you might even say how everyone you told this to asked about exchange relations.
In any case, you could say that coefficient-free exchange relations were your other result (quoted right now as the other half of \cref{prop:coeff-free} and that the $y^\alpha$ thing was also immediate corollary of your awesome result.
Determining the $\alpha$ uniquely was hard!  (I remember hearing about it!)
}
  ...
  \saySS{Write something about the Grey fellowship here.}
...
\section{Proof of \cref{thm:main}}
  ...
  \begin{corollary}
    ...
    If \cref{eq:system} has more than one solution among the positive roots of $A$ then it has precisely two. \say[N]{fixed "then" and "than"}
  ...
  Any cluster algebra of type $D_n$ can be realized as triangulated once-punctured disk. 
  Since we are only considering acyclic initial seeds the lamination encoding the initial triangulation will contain a digon with one side on the boundary of the disk (cf. \cref{fig:D_n-roots}).
  By reflecting our surface, if necessary, we can always assume that $n-2 \prec_c n-1$; it will therefore suffices to consider only two cases: either $n-2 \prec_c n$, or $n \prec_c n-2$.
  ...
  \begin{proof}
    ...
    An easy rule, though, can be obtained using \cite[Eq. (1.13)]{NZ11}: it suffices to reflect our surface and compute the shear coordinates of the elementary lamination corresponding to the desired arc with respect to the initial triangulation.
    \saySS{\label{note}Pavel you said that this is written already somewhere, do you have a reference to add here?}
\say[NR]{About Note~\ref{note}, you can find this, said another way, as Proposition 5.2 of my paper "Universal geometric cluster algebras from surfaces" but it wouldn't surprise me if someone else wrote it down earlier. }
    \saySS{We do not care much about spiralling directions because we only care about the absolute value of the coordinates; it might be a good idea though to check that \cite{NZ11} gives plays nicely with our conventions.}
  ...
  \]
  Therefore, since the pairing $\langle\bullet,\bullet\rangle$ is integer-valued when computed on (co-)roots and weights, $\alpha$ and $\alpha'$ cannot both satisfy \cref{eq:dual}.

  \begin{remark}
    A modified version of \cref{prop:coeff-free} holds in affine types as well.
    \saySS{@Nathan: should we mention more explicitly our work in progress on this?}
    \say[NR]{Feel free to say as much as we know if you think it fits in.}
    We expect \cref{thm:main} to hold there too.
    The analysis required to establish it, though, could be much more complicated than the finite case one because the corank of $B_c$ can be as big as $4$ and the argument of \cref{rk:induction} does not apply.
  \end{remark}

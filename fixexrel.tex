...
\section{Introduction}
  Given two cluster variables in a cluster algebra, deciding whether they belong to the same cluster or if they can be obtained from one another by a single mutation is, in general, an hard problem to address.
  ...
  In \cite{Ste13} the second author gave a uniform formula for all the exchange relations, albeit only in the coefficient-free case.
  Our main theorem here improves on this result to deal with principal coefficients as well.
 ...
 In order to make this statement more precise we need to recall few notions and results from \cite{Ste13,YZ08}.
  ...
  We will denote by $\cA(c)$ 
  \say[NR]{You probably want $\cA_\bullet(c)$ here?} 
  \saySS{Not sure, if I am not mistaken \cite{YZ08} uses this notation. Anyone has a preference? I prefer $\cA_\bullet(c)$} 
  ...
  This is a combinatorial shadow of a notable automorphism of the coefficient-free conterpart of $\cA(c)$ sending the cluster variable $x_\lambda$ to $x_{\tau_c(\lambda)}$. 
  ...
  \end{definition}
  The name comes from the following important property, consequence of the polytopal realization of the cluster fan of $\cA(c)$ (\cite{CFZ02,Ste13}).
  \begin{proposition}
    \say[NR]{This theorem should be cited to a specific result (if possible---this kind of statement has been really hard for me to track down in the past!}
    \saySS{My bad: this proposition follows basically from how you prove the main theorem in \cite{Ste13}; unfortunately I was too dumb at the time to realize that this was going to be important and I did not state it in any quotable manner there. To my defence I can only say that even in \cite{CFZ02} they do not give a quotable statement.}
  ...
  Let $y_1,\dots,y_n$ be the generators of the coefficient semifield of $\cA(c)$ and denote by $y^\alpha$ the product $\prod_{i=1}^n y_i^{[\alpha;\alpha_i]}$.
  \begin{theorem}
    \label{thm:main}
    Suppose $\lambda$ and $\mu$ are exchangeable weights in $\Pi(c)$. 
    Then the cluster variables $x_\lambda$ and $x_\mu$ in $\cA(c)$ satisfy the exchange relation
    \begin{equation}
      \label{eq:exrel}
      x_\lambda x_\mu = x_{\lambda+\mu} + y^\alpha x_{\lambda\uplus_c\mu}
    \end{equation}
    where $\alpha$ is the unique positive root in the root system of $A$ such that
    \begin{equation}
      \label{eq:system}
    ...
    \begin{equation}
      ...
      = -1.
    \end{equation}
    (Here $\alpha^\vee$ denotes the \emph{coroot} corresponding to $\alpha$ while $\langle\bullet,\bullet\rangle$ is the pairing of dual vector spaces.)
  \end{theorem}

  Note that the shape of \cref{eq:exrel} follows imediately form the coefficient-free case \cite[Proposition 5.2]{Ste13} together with the observations that $\bc$-vectors are roots in the root system of $A$ (cf. \cite{NS14})  and that the exchange relations in $\cA(c)$ are homogeneous.
  The real content of our theorem are therefore the explicit conditions that determine $\alpha$. 
  ...
  This paper was completed during a visit to Durham University; the second author would like to thank both Grey College and the Department of Mathematics for the hospitality received.
  We are also grateful to Nathan Reading for many fruitful discussions, for his help with some hearly attempts at \cref{thm:main}, and for his comments on a preliminary version of this paper.
...
\say[NR]{About Note~\ref{note}, you can find this, said another way, as Proposition 5.2 of my paper "Universal geometric cluster algebras from surfaces" but it wouldn't surprise me if someone else wrote it down earlier. }
  ...
  \begin{remark}
    In view of some ongoing work of the second author with Nathan Reading it appears that a modified version of \cite[Propositions 5.1 and 5.2]{Ste13} holds in affine types as well.
